...
SEARCH_PATH = os.path.normpath(str(BASE_DATA_PATH / "{dataset}/{dataset}.zarr/recon-1/{name}"))
CROP_NAME = os.path.normpath("labels/groundtruth/{crop}/{label}")
RAW_NAME = os.path.normpath("em/fibsem-uint8")
PREDICTIONS_PATH = os.path.normpath(str(BASE_DATA_PATH / "predictions/{dataset}.zarr/{crop}"))
PROCESSED_PATH = os.path.normpath(str(BASE_DATA_PATH / "predictions/{dataset}.zarr/{crop}"))
